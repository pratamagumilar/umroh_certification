...
export default function PanitiaMaterialsPage() {
  ...
  const fetchMaterials = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/panitia/materials');
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/panitia/materials/${selectedMaterial.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          title: formTitle,
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 800, color: '#1a201b', mb: 0.5 }}>
            Master Materi Reusable (Panitia)
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Kelola materi PDF global yang dapat dimasukkan ke dalam Course pembelajaran.
          </Typography>
        </Box>
        <Button
          variant="contained"
